...
func TestNewMqttFactory(t *testing.T) {
	...
	target := NewMqttFactory(secretDataProvider, lc, expectedMode, expectedPath, expectedSkipVerify, RetrySettings{})
...
func TestConfigureMQTTClientForAuth(t *testing.T) {
	target := NewMqttFactory(secretDataProvider, lc, "", "", false, RetrySettings{})
...
func TestNewRetrySettingsSecure(t *testing.T) {
	err := os.Setenv(secret.EnvSecretStore, "true")
	defer os.Clearenv()
	require.NoError(t, err)
	expectedDuration := 10
	expectedInterval := 1
	tests := []struct {
		name             string
		target           RetrySettings
		expectedDuration int
		expectedInterval int
	}{
		{"default", NewRetrySettings(0, 0), defaultRetryDuration, defaultRetryInterval},
		{"custom", NewRetrySettings(expectedDuration, expectedInterval), expectedDuration, expectedInterval},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			assert.True(t, test.target.Retry)
			assert.Equal(t, test.expectedDuration, test.target.Duration)
			assert.Equal(t, test.expectedInterval, test.target.Interval)
		})
	}
}

func TestNewRetrySettingsInSecure(t *testing.T) {
	err := os.Setenv(secret.EnvSecretStore, "false")
	defer os.Clearenv()
	require.NoError(t, err)
	target := NewRetrySettings(10, 1)
	assert.False(t, target.Retry)
}
...
func TestGetValidSecretData(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			f := NewMqttFactory(secretDataProvider, lc, test.AuthMode, test.SecretPath, false, RetrySettings{})
